...
async function checkMobileCoreScreenRouting(page) {
  const result = await page
    .evaluate(async () => {
      ...
      // KYC hard-gate must redirect blocked pages.
      window.kycGateActive = true;
      window.showScreen("market", false);
      await sleep(120);
      const certCenter = document.getElementById("certification-center-screen");
      if (!certCenter || certCenter.classList.contains("hidden")) {
        return fail("kyc gate did not redirect market -> certification-center");
      }

      return { ok: true, reason: "mobile screen routing and KYC gate ok" };
    })
    .catch((err) => ({ ok: false, reason: err.message }));
